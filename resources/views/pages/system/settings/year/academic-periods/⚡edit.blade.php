...
new #[Title('Editar Periodo Academico')] class extends Component {
    public ?AcademicPeriod $period = null;
    ...
    {
        $this->period = AcademicPeriod::findOrFail($id);

        $this->fill([
            'trimester_name' => $this->period->trimester_name,
            'year_id' => $this->period->year_id,
            'start_date' => $this->period->start_date?->format('Y-m-d'),
            'end_date' => $this->period->end_date?->format('Y-m-d'),
            'grading_open_date' => $this->period->grading_open_date?->format('Y-m-d'),
            'grading_close_date' => $this->period->grading_close_date?->format('Y-m-d'),
            'is_supletorio' => $this->period->is_supletorio,
        ]);
    }
    ...
    protected function rules(): array
    {
        return [
            ...
            'grading_close_date' => ['nullable', 'date'],
    ...
    public function update(): void
    {
        ...
        Flux::toast(variant: 'success', text: __('Periodo academico actualizado correctamente.'));
...
<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">{{ __('Editar Periodo Academico') }}</flux:heading>
            <flux:text variant="subtle" class="mt-1">{{ __('Actualizar informacion del periodo') }}</flux:text>
        </div>
    ...
    <form wire:submit="update" class="space-y-6 max-w-3xl">
        <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <flux:field>
                    ...
                    <flux:input wire:model="trimester_name" />
        ...
        <div class="flex gap-3">
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">{{ __('Actualizar Periodo') }}</flux:button>

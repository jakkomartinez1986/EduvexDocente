...
new #[Title('Crear Dia de Calendario')] class extends Component {
    ...
    protected function rules(): array
    {
        return [
            ...
            'date' => ['required', 'date', 'date_format:Y-m-d', 'unique:calendar_days,date,NULL,id,year_id,' . $this->year_id],
    ...
    public function save(): void
    {
        ...
        Flux::toast(variant: 'success', text: __('Dia de calendario creado correctamente.'));
...
<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">{{ __('Crear Dia de Calendario') }}</flux:heading>
            <flux:text variant="subtle" class="mt-1">{{ __('Registrar un nuevo dia en el calendario escolar') }}</flux:text>
        </div>
    ...
    <form wire:submit="save" class="space-y-6 max-w-3xl">
        ...
        <div class="flex gap-3">
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">{{ __('Guardar Dia') }}</flux:button>

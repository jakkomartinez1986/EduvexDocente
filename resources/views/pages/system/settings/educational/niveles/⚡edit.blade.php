...
new #[Title('Editar Nivel')] class extends Component {
    ...
    {
        $this->nivel = Nivel::findOrFail($id);

        $this->fill([
            'nivel_name' => $this->nivel->nivel_name,
            'shift_id' => $this->nivel->shift_id,
        ]);
    }
    ...
    {
        return [
            'nivel_name' => ['required', 'string', 'max:100', 'unique:nivels,nivel_name,' . $this->nivel?->id],
    ...
    public function update(): void
    {
        ...
        Flux::toast(variant: 'success', text: __('Nivel actualizado correctamente.'));
...
<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">{{ __('Editar Nivel') }}</flux:heading>
            <flux:text variant="subtle" class="mt-1">{{ __('Actualizar informacion del nivel') }}</flux:text>
        </div>
    ...
    <form wire:submit="update" class="space-y-6 max-w-3xl">
        ...
        <div class="flex gap-3">
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">{{ __('Actualizar Nivel') }}</flux:button>

...
new #[Title('Mis Estudiantes')] class extends Component {
    use WithPagination;
    ...
    public function confirmToggle(int $id): void
    {
        ...
        $this->dispatch('showConfirm',
            ...
            eventName: 'execute-toggle-tutor-student',
            eventParams: []
        )->to('confirm-action');
    }

    #[On('execute-toggle-tutor-student')]
    public function executeToggleStudent(): void
    {
        $student = Student::find($this->toggleId);
        if ($student) {
    ...
    {
        $this->dispatch('showConfirm',
            message: 'Esta seguro de activar a todos los estudiantes de este grado?',
            eventName: 'execute-activate-all',
            eventParams: []
        )->to('confirm-action');
    }

    #[On('execute-activate-all')]
    public function executeActivateAll(): void
    {
        ...
        $studentIds = \App\Models\Management\Enrollments\StudentEnrollment::where('year_id', $yearId)
        ...
        Student::whereIn('id', $studentIds)
            ->whereHas('user', fn ($q) => $q->where('status', '!=', 1))
            ->each(fn ($student) => $student->user?->update(['status' => 1]));

        Flux::toast(
            variant: 'success',
            text: "{$count} estudiante(s) activado(s) correctamente."
        );
    }
    ...
    public function getStudentCount(): int
    {
        ...
        return \App\Models\Management\Enrollments\StudentEnrollment::where('year_id', $yearId)
            ->where('grade_id', $tutorSchedule->grade_id)
            ->where('status', 'active')
            ->count();
    }
...
<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">{{ __('Mis Estudiantes') }}</flux:heading>
            <flux:text variant="subtle" class="mt-1">{{ __('Estudiantes del grado tutorado') }}</flux:text>
        </div>
        @if($found)
            <div class="flex gap-2">
                <flux:button href="{{ route('system.identity.students.import', $gradeId) }}" wire:navigate variant="outline" icon="arrow-up-tray">
                    {{ __('Importar') }}
                </flux:button>
    ...
    <nav class="mb-6 flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400" aria-label="Breadcrumb">
        ...
        <span>/</span>
        <span class="text-zinc-900 dark:text-zinc-100 font-medium">{{ __('Mis Estudiantes') }}</span>
    </nav>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-400">
                        @if($shiftName)
                            <span>{{ $shiftName }}</span>
                        @endif
                        <span>{{ $this->getStudentCount() }} {{ __('estudiantes matriculados') }}</span>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        @forelse ($this->records as $student)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <flux:button href="{{ route('system.identity.students.show', $student->id) }}" wire:navigate size="sm" variant="ghost" icon="eye">
                                        {{ __('Ver') }}
                                    </flux:button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-16 text-center">
                                    ...
                                    <flux:text variant="subtle" class="text-sm">{{ __('No se encontraron estudiantes en este grado.') }}</flux:text>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>

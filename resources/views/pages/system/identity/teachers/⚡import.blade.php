...
new #[Title('Importar Docentes')] class extends Component {
    use WithFileUploads;
    ...
    {
        $user = auth()->user();
        if ($user->hasAnyRole(['DOCENTE', 'TUTOR', 'REPRESENTANTE', 'ESTUDIANTE'])) {
            abort(403, 'No tiene permisos para importar docentes.');
        }
    }
...
<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">{{ __('Importar Docentes') }}</flux:heading>
            <flux:text variant="subtle" class="mt-1">{{ __('Cargar docentes desde un archivo Excel') }}</flux:text>
        </div>
    ...
    @if (!$this->showPreview)
        ...
        <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-6 max-w-2xl">
            <div class="mb-4">
                ...
                <flux:text variant="subtle">{{ __('Seleccione el archivo Excel con los datos de los docentes a importar.') }}</flux:text>
            </div>
            ...
            <form wire:submit="processFile">
                ...
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <a href="{{ route('system.identity.templates.download', 'docentes') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 transition">
            ...
            <div class="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-4">
                <div class="flex gap-3">
                    ...
                    <div class="text-sm text-blue-700 dark:text-blue-300">
                        ...
                        <p>{{ __('El archivo debe contener las columnas:') }} <strong>NOMBRES,APELLIDOS,DNI,EMAIL,TELEFONO,CELULAR,DIRECCION, ESPECIALIZACION, TITULO, NIVEL_EDUCATIVO, FECHA_INGRESO</strong></p>
                        <p class="mt-1">{{ __('Los codigos de docente se generaran automaticamente.') }}</p>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50">
                            ...
                            <th class="px-3 py-2 text-left font-medium text-zinc-600 dark:text-zinc-400">{{ __('Email') }}</th>
                            <th class="px-3 py-2 text-left font-medium text-zinc-600 dark:text-zinc-400">{{ __('Celular') }}</th>
                            <th class="px-3 py-2 text-left font-medium text-zinc-600 dark:text-zinc-400">{{ __('Especializacion') }}</th>
                    ...
                    <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                        @foreach ($previewData as $index => $row)
                            <tr class="{{ isset($row['errors']) ? 'bg-red-50 dark:bg-red-900/10' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50' }}">
                                ...
                                <td class="px-3 py-2 text-zinc-700 dark:text-zinc-300">{{ $row['email'] ?? '-' }}</td>
                                <td class="px-3 py-2 text-zinc-700 dark:text-zinc-300">{{ $row['cellphone'] ?? '-' }}</td>
                                <td class="px-3 py-2 text-zinc-700 dark:text-zinc-300">{{ $row['specialization'] ?? '-' }}</td>
                                <td class="px-3 py-2">
                                    @if (isset($row['errors']))

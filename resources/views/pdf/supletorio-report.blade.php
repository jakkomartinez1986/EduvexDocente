...
<head>
    ...
    <style>
        ...
        .matrix-table .name-col { text-align: left; min-width: 130px; }
        ...
        .supletorio-col { background-color: #fef2f2; }
    ...
    <table class="matrix-table">
        <thead>
            <tr>
                ...
                <th>{{ __('1er Trimestre') }}<div style="font-size:6.5px;font-weight:normal;">{{ __('Total') }}</div></th>
                <th>{{ __('2do Trimestre') }}<div style="font-size:6.5px;font-weight:normal;">{{ __('Total') }}</div></th>
                <th>{{ __('3er Trimestre') }}<div style="font-size:6.5px;font-weight:normal;">{{ __('Total') }}</div></th>
                <th class="total-col">{{ __('Prom. Anual') }}</th>
                <th class="supletorio-col">{{ __('Supletorio') }}</th>
                <th>{{ __('Estado Final') }}</th>
            </tr>
        </thead>
        <tbody>
            @forelse($studentsData as $sData)
                @php
                    $validTotals = array_filter($sData['trimesters'], fn($v) => $v !== null);
                    $annualAvg = count($validTotals) > 0
                        ? round(array_sum($validTotals) / count($validTotals), 2)
                        : null;

                    $supGrade = $sData['supletorio'];
                    $aprobadoSupletorio = $supGrade !== null && $supGrade >= 7;

                    $finalStatus = $aprobadoSupletorio ? 'Aprobado por Supletorio'
                        : ($annualAvg !== null
                            ? ($annualAvg >= 7 ? 'Aprobado' : ($annualAvg >= 5 ? 'Supletorio' : 'Reprobado'))
                            : 'Sin datos');
                    $finalStatusClass = $aprobadoSupletorio ? 'status-aprobado'
                        : ($annualAvg !== null
                ...
                <tr>
                    ...
                    <td style="font-weight:bold;">{{ $sData['trimesters'][0] !== null ? number_format($sData['trimesters'][0], 2) : '—' }}</td>
                    <td style="font-weight:bold;">{{ $sData['trimesters'][1] !== null ? number_format($sData['trimesters'][1], 2) : '—' }}</td>
                    <td style="font-weight:bold;">{{ $sData['trimesters'][2] !== null ? number_format($sData['trimesters'][2], 2) : '—' }}</td>
                    <td class="total-col">{{ $annualAvg !== null ? number_format($annualAvg, 2) : '—' }}</td>
                    <td class="supletorio-col" style="font-weight:bold;">{{ $supGrade !== null ? number_format($supGrade, 2) : '—' }}</td>
                    <td class="{{ $finalStatusClass }}">{{ $finalStatus }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" style="text-align:center;">{{ __('No hay datos de calificaciones.') }}</td>
                </tr>
            @endforelse
        </tbody>
    </table>

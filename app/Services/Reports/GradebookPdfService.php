...
/**
 * Render de los reportes PDF del gradebook (resúmenes por asignatura y reportes
 * del tutor). Centraliza el cómputo que antes vivía en GradebookPdfController
 * para que el job asíncrono GeneratePdfReport lo re-ejecute sin depender de
 * auth(): el contexto serializable (ids + teacher_id/teacher_name) lleva todo
 * lo necesario y el worker resuelve el año lectivo activo por su cuenta.
 *
 * Cada método devuelve ['binary' => string, 'filename' => string] y filename()
 * expone el nombre determinístico del archivo para el chequeo previo al
 * dispatch. assertDispatchable() reproduce los abort(404) que el controlador
 * emitía en la petición.
 */
class GradebookPdfService
{
    public const FORMATIVE = 'gradebook_formative';

    public const SUMMATIVE = 'gradebook_summative';

    public const QUALITATIVE = 'gradebook_qualitative';

    public const SUBJECT_ANNUAL = 'gradebook_subject_annual';

    public const SUPLETORIO = 'gradebook_supletorio';

    public const TUTOR_STUDENT = 'gradebook_tutor_student';

    public const TUTOR_STUDENT_TRI = 'gradebook_tutor_student_tri';

    public const TUTOR_FORMATIVE_TRI = 'gradebook_tutor_formative_tri';

    public const TUTOR_ALL_TRI = 'gradebook_tutor_all_tri';

    public const STUDENT_TRI = 'gradebook_student_tri';

    public const STUDENT_ANNUAL = 'gradebook_student_annual';

    /** @var list<string> */
    private const TUTOR_TYPES = [
        self::TUTOR_STUDENT, self::TUTOR_STUDENT_TRI, self::TUTOR_FORMATIVE_TRI,
        self::TUTOR_ALL_TRI, self::STUDENT_TRI, self::STUDENT_ANNUAL,
    ];
    ...
    /**
     * Reproduce los abort(404) que el controlador emitía en la petición para no
     * encolar trabajo imposible (sin asignación, estudiante o trimestre).
     *
     * @param  array<string, mixed>  $ctx
     */
    public function assertDispatchable(string $type, array $ctx): void
    {
        ...
        if (in_array($type, self::TUTOR_TYPES, true)) {
            ...
            if (isset($ctx['student_id'])) {
                $student = Student::where('id', $ctx['student_id'])
                    ->whereHas('enrollments', fn ($q) => $q->where('grade_id', $tutorSchedule->grade_id)->where('year_id', $yearId))
                    ->first();

                abort_if(! $student, 404, __('Estudiante no encontrado en el grado del tutor.'));
            }

            if (isset($ctx['trimester_id'])) {
                $period = AcademicPeriod::find($ctx['trimester_id']);
    ...
    {
        $safe = static fn (mixed $value): string => str_replace(['/', '\\', ':'], '-', (string) $value);
        ...
        if (in_array($type, self::TUTOR_TYPES, true)) {
            ...
            return match ($type) {
                ...
                self::TUTOR_ALL_TRI => 'Notas_Todos_'.$safe($r['gradeName']).'_'.$safe($r['period']->trimester_name).'.pdf',
        ...
        return match ($type) {
            ...
            self::SUBJECT_ANNUAL => 'Informe_Anual_'.$safe($r['subjectName']).'_'.$safe($r['gradeName']).'.pdf',
     ...
     */
    public function formative(array $ctx): array
    {
        $data = $this->buildContext($ctx);
     ...
     * @return array{binary: string, filename: string}
     */
    public function summative(array $ctx): array
    {
        $data = $this->buildContext($ctx);
     ...
     * @return array{binary: string, filename: string}
     */
    public function qualitative(array $ctx): array
    {
        $data = $this->buildContext($ctx);
     ...
     * @return array{binary: string, filename: string}
     */
    public function subjectAnnual(array $ctx): array
    {
        $teacherId = $ctx['teacher_id'];
        $yearId = $this->years->getActiveYearId();
        ...
        $students = Student::whereHas('enrollments', fn ($q) => $q->where('grade_id', $ctx['grade_id'])->where('year_id', $yearId))
            ->with('user')
            ->orderBy(User::select('lastname')->whereColumn('users.id', 'students.user_id'))
            ->get();

        $subjectId = $ctx['subject_id'];
        $gradeId = $ctx['grade_id'];
        ...
        return [
            'binary' => $this->renderPdf('pdf.subject-annual-report', $viewData, 'a4', 'portrait'),
            'filename' => $filename,
        ];
    }
     ...
     * @return array{binary: string, filename: string}
     */
    public function supletorio(array $ctx): array
    {
        $teacherId = $ctx['teacher_id'];
        $yearId = $this->years->getActiveYearId();
        ...
        return [
            'binary' => $this->renderPdf('pdf.supletorio-report', $viewData, 'a4', 'portrait'),
            'filename' => $filename,
        ];
    }
     ...
     * @return array{binary: string, filename: string}
     */
    public function tutorStudent(array $ctx): array
    {
        $teacherId = $ctx['teacher_id'];
        $yearId = $this->years->getActiveYearId();

        $tutorSchedule = $this->tutorSchedule($teacherId, $yearId);
        ...
        return [
            'binary' => $this->renderPdf('pdf.tutor-student-report', $viewData, 'a4', 'portrait'),
            'filename' => $filename,
        ];
    }
     ...
     * @return array{binary: string, filename: string}
     */
    public function tutorStudentTri(array $ctx): array
    {
        $teacherId = $ctx['teacher_id'];
        $yearId = $this->years->getActiveYearId();
        ...
        return [
            'binary' => $this->renderPdf('pdf.tutor-student-report-trimester', $viewData, 'a4', 'portrait'),
            'filename' => $filename,
        ];
    }
     ...
     * @return array{binary: string, filename: string}
     */
    public function tutorFormativeTri(array $ctx): array
    {
        $teacherId = $ctx['teacher_id'];
        $yearId = $this->years->getActiveYearId();
     ...
     * @return array{binary: string, filename: string}
     */
    public function tutorAllTri(array $ctx): array
    {
        $teacherId = $ctx['teacher_id'];
        $yearId = $this->years->getActiveYearId();
        ...
        $gradeId = $tutorSchedule->grade_id;
        $selectedPeriod = AcademicPeriod::find($ctx['trimester_id']);
        ...
        return [
            'binary' => $this->renderPdf('pdf.tutor-all-students-trimester', $viewData, 'a4', 'portrait'),
            'filename' => $filename,
        ];
    }
     ...
     * @return array{binary: string, filename: string}
     */
    public function studentTri(array $ctx): array
    {
        $teacherId = $ctx['teacher_id'];
        $yearId = $this->years->getActiveYearId();
        ...
        return [
            'binary' => $this->renderPdf('pdf.student-trimester-report', $viewData, 'a4', 'portrait'),
            'filename' => $filename,
        ];
    }
     ...
     * @return array{binary: string, filename: string}
     */
    public function studentAnnual(array $ctx): array
    {
        $teacherId = $ctx['teacher_id'];
        $yearId = $this->years->getActiveYearId();
        ...
        return [
            'binary' => $this->renderPdf('pdf.student-annual-report', $viewData, 'a4', 'portrait'),
            'filename' => $filename,
        ];
    }

    /**
     * Búsquedas baratas compartidas para validación y nombres de archivo.
     *
     * @param  array<string, mixed>  $ctx
     * @return array{year_id: int, tutorSchedule: object, student: object|null, period: object|null, gradeName: string}
     */
    protected function tutorResolved(array $ctx): array
    {
        $yearId = $this->years->getActiveYearId();
        $teacherId = $ctx['teacher_id'];
    ...
    /**
     * @param  array<string, mixed>  $ctx
     * @return array{year_id: int, subjectName: string, gradeName: string, period: object|null}
     */
    protected function subjectResolved(array $ctx): array
    {
        $yearId = $this->years->getActiveYearId();

        $schedule = ClassSchedule::where('teacher_id', $ctx['teacher_id'])
            ->where('subject_id', $ctx['subject_id'])
            ->where('grade_id', $ctx['grade_id'])
            ->where('year_id', $yearId)
            ->with('grade', 'subject')
        ...
        $period = isset($ctx['trimester_id']) ? AcademicPeriod::find($ctx['trimester_id']) : null;
        abort_if($period && $period->is_supletorio, 404, __('No se encontró el trimestre.'));

        return [
            'year_id' => $yearId,
            'subjectName' => $schedule->subject->subject_name ?? '',
            'gradeName' => trim(($schedule->grade->grade_name ?? '').' '.($schedule->grade->section ?? '')),
            'period' => $period,
        ];
    }

    protected function tutorSchedule(int $teacherId, int $yearId): ?ClassSchedule
    {
    ...
    /**
     * Contexto base para los reportes por asignatura (formative/summative/
     * qualitative): datos de cabecera más la lista de estudiantes del grado.
     *
     * @param  array<string, mixed>  $ctx
     * @return array<string, mixed>
     */
    protected function buildContext(array $ctx): array
    {
        $yearId = $this->years->getActiveYearId();
        $teacherId = $ctx['teacher_id'];
        ...
        $period = AcademicPeriod::find($ctx['trimester_id'] ?? 0);
        abort_if(! $period || $period->is_supletorio, 404, __('No se encontró el trimestre.'));

        $studentQuery = Student::whereHas('enrollments', fn ($q) => $q->where('grade_id', $ctx['grade_id'])->where('year_id', $yearId))
            ->with('user')
            ->orderBy(User::select('lastname')->whereColumn('users.id', 'students.user_id'))
            ->orderBy(User::select('name')->whereColumn('users.id', 'students.user_id'));

        if (! empty($ctx['student_id'])) {
            $studentQuery->where('id', $ctx['student_id']);
        }
     ...
     * @return array<string, mixed>
     */
    protected function buildQualitativeData(string $type, Subject $subject, int $gradeId, int $studentId, int $yearId, $periods): array
    {
    ...
    /**
     * Render con las mismas opciones de página que el controlador previo.
     */
    protected function renderPdf(string $view, array $data, string $paper, string $orientation): string
    {
